...
def execute_noAPI(df, index_client, model):
    ...
    predict_proba = model.predict_proba(
        [df.drop(columns="TARGET").fillna(0).loc[index_client]]
    )[:, 1]
    predict_target = (predict_proba >= 0.4).astype(int)
    col1, col2, col3 = st.columns(3)
    col1.metric(
        "Difficulties", str(np.where(df["TARGET"].loc[index_client] == 0, "NO", "YES"))
    )
    col2.metric(
        "Predicted Difficulties", str(np.where(predict_target == 0, "NO", "YES"))[2:-2]
    )
...
def execute_API(df):
    ...
    request = requests.post(
        ...
        headers={
            ...
            "Authorization": "Bearer vbcf@X9rVvo9NEhkEWevn6$rQBjkNbQs4ovD5MWX$zzic589ic4S&6LEkgoYtDD%69t",
        },
    )
...
def plot_client(df, explainer, df_reference, index_client=0):
    ...
    shap_importance = (
        pd.Series(shap_values, df.columns).abs().sort_values(ascending=False)
    )
    ...
    with col1:
        for features in list(shap_importance.index[:6])[:3]:
            ...
            if df_reference[features].nunique() == 2:
                ...
                plt.scatter(
                    y=df[features].loc[index_client] + 0.1,
                    x=features,
                    marker="o",
                    s=100,
                    color="r",
                ...
                figInd.annotate(
                    "Client ID:\n{}".format(index_client),
                    xy=(features, df[features].loc[index_client] + 0.1),
                    xytext=(0, 40),
            ...
            # For non binary features :
                ...
                figInd.annotate(
                    "Client ID:\n{}".format(index_client),
                    xy=(0.5, df[features].loc[index_client]),
                    xytext=(0, 40),
                    textcoords="offset points",
                    ha="center",
                    va="bottom",
                ...
                figInd.axhline(
                    y=df_reference[df_reference["TARGET"] == 0][features].mean(),
                    zorder=0,
                    linestyle="--",
                    color="#1f77b4",
                )
                figInd.axhline(
                    y=df_reference[df_reference["TARGET"] == 1][features].mean(),
                    zorder=0,
                    linestyle="--",
                    color="#ff7f0e",
                )
...
def nan_values(df, index_client=0):
    if np.isnan(df.loc[index_client]).any():
        ...
        with col1:
            st.table(
                data=pd.DataFrame(nan_col, columns=["FEATURES WITH MISSING VALUES"])
            )
        with col2:
